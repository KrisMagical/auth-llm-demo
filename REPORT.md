...
当前项目已经完成本地功能、页面、Docker 文件、部署说明和最终验收清单。公网部署需要在实际云平台完成后填写真实公网地址，当前不编造 URL。
...
设计出发点是优先保证题目核心验收点：注册、登录、JWT、资源 A/B 授权、资源 B 测试账号和 LLM username checker。权限链路需要清晰可验证；LLM 判断不作为强依赖，因此提供 mock fallback 和 failed fallback；Demo 保持轻量，方便部署和评审。

## 3. AI Coding 工具使用情况

本项目使用 ChatGPT / Codex 作为辅助工具，但使用范围主要集中在简单代码框架生成、阶段任务拆解、文档措辞检查和测试用例审校。

AI 主要负责：

1. 根据需求生成初始 FastAPI 项目结构和部分基础样板代码。
2. 辅助整理 README、REPORT、部署说明和验收清单。
3. 辅助审校测试覆盖是否遗漏关键路径，例如普通用户访问资源 B 返回 403、admin 访问资源 B 成功、LLM / mock fallback 不影响注册主流程。
4. 辅助检查安全边界说明，例如不提交 `.env`、不写死 API Key、不编造公网 URL。

Token 数为估算，不做精确统计。整体大约使用 60k - 100k tokens，主要消耗在需求拆解、基础框架生成、测试点审校和 README / REPORT 文档整理上。

## 4. 自己时间花最多的部分

不计算模型生成时间，自己花最多时间的部分是核心业务逻辑编写、功能联调、部署环境配置和验收结果核对。尤其是认证与授权链路需要逐项确认：普通注册用户必须默认是 `role=user`，普通用户可以访问资源 A，普通用户访问资源 B 必须稳定返回 403，admin 测试账号必须可以登录并访问资源 B。

LLM 用户名审核部分也花了较多时间人工确认，包括没有 API Key 时是否稳定走 mock checker，DeepSeek / OpenAI-compatible provider 的环境变量是否清晰，LLM 返回非法 JSON 或调用异常时是否保存 failed 并允许注册，以及 rejected / mock_rejected 是否确实会拒绝注册。

页面和部署部分主要由我自己做实际验证：检查浏览器能否完成注册、登录、查看当前用户、访问资源 A/B；检查服务器上的 `.env`、DeepSeek API Key、uvicorn、Apache 反向代理和公网访问流程是否可按文档复现。AI 在这些部分主要起到审校和提示遗漏项的作用。
...
- 没有 `OPENAI_API_KEY` 或 `DEEPSEEK_API_KEY` 时使用 mock checker。
...
- 也可以在云服务器上使用 Apache 反向代理到本机 uvicorn 服务。
- 生产环境变量必须通过云平台或服务器环境变量配置，不提交真实 `.env`。
- `OPENAI_API_KEY` / `DEEPSEEK_API_KEY` 可选；未配置时使用 mock checker，保证 Demo 可运行。
...
- `JWT_SECRET`、`ADMIN_PASSWORD`、`OPENAI_API_KEY`、`DEEPSEEK_API_KEY` 必须通过环境变量配置。
- `.env` 不应提交到 GitHub。
- 生产环境应使用 HTTPS。
- LLM 用户名审核不作为严格内容安全系统。

## 11. DeepSeek Provider Update

项目现在除 OpenAI-compatible provider 外，也支持 DeepSeek provider。DeepSeek provider 仍然遵循同样的结构化 JSON 输出：`allowed`、`rejected`、`uncertain`。如果 DeepSeek 返回非法 JSON 或调用异常，系统保存 `failed`，不会让注册接口 500。
...
如果没有配置 `DEEPSEEK_API_KEY`，系统仍然走 mock fallback，保证 Demo 可运行。本文档不写真实 API Key，也不声称已经真实调用 DeepSeek，除非实际部署环境配置 key 后完成验证。
